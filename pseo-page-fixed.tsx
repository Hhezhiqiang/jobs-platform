...
const CITY_NAME_TO_EN: Record<string, string> = {
  ...
  "南京": "Nanjing", "苏州": "Suzhou", "远程": "Remote"
};
...
const CITY_INTRO: Record<string, string> = {
  ...
  "西安": "西北地区科技中心，航空航天、软件开发产业根基深厚。",
  "南京": "科教资源丰富，软件与信息服务业发达。",
  "苏州": "制造业和工业园区闻名，在生物医药、人工智能领域加速布局。",
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = isEn
    ...
    : "查看" + cityName + typeLabel + "招聘信息，" + typeLabel + "岗位汇总。" + cityIntro.slice(0, 100) + "实时更新。";
  const url = SITE_URL + "/" + locale + "/jobs/city/" + encodeURIComponent(city) + "/" + encodeURIComponent(type);
  return {
    title, description,
    keywords: isEn
      ? [cityName + " " + typeLabel + " jobs", cityName + " hiring"]
      : [cityName + typeLabel + "招聘", cityName + "高薪职位"],
...
export default async function CityTypeJobsPage({ params }: PageProps) {
  ...
  if (city && city !== "all" && city !== "") {
    ...
    whereClause.OR = [{ city: city }, { city: zhCity }];
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {jobs.length === 0 ? (
          ...
          <div className="space-y-4">
            {jobs.map((job) => <JobCardV2 key={job.id} job={job} locale={locale} />)}
          </div>
        )}
