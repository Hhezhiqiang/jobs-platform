...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jobIds || !Array.isArray(jobIds) || jobIds.length === 0) {
      return NextResponse.json({ error: "请选择至少一个职位" }, { status: 400 });
    }
      ...
      try {
        ...
        await prisma.job_applications.create({
          data: {
            ...
            guestName: isGuest ? name || undefined : undefined,
    ...
    return NextResponse.json({
      ...
      skipped,
      total: jobIds.length,
      message: "已申请 " + success + " 个职位",
    });
  } catch (error) {
    return NextResponse.json({ error: "批量申请失败" }, { status: 500 });
  }
}
